  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2 text-xs">
          <button
            ...
            className={`px-3 py-1 rounded-full ${mode === 'cover' ? 'bg-primary-subtle text-primary' : 'bg-surface-muted'}`}
          >
            封面
          </button>
          <button
            onClick={() => setMode('content')}
            className={`px-3 py-1 rounded-full ${mode === 'content' ? 'bg-primary-subtle text-primary' : 'bg-surface-muted'}`}
          >
            正文
          </button>
        </div>
      ...
      <div className="relative group">
        <div className={`flex justify-center bg-surface-muted p-4 rounded-lg overflow-hidden border border-border ${hideControls ? 'p-0 border-0 bg-transparent' : ''}`}>
          ...
          >
            ...
            {mode === 'cover' ? (
              ...
              <div className="flex flex-col h-full justify-center items-center text-center space-y-6 z-10 relative">
                 ...
                 <h1 className={`text-3xl font-bold leading-tight drop-shadow-lg ${showBackgroundImage ? 'text-primary-text' : ''}`} style={{ textShadow: showBackgroundImage ? '0 2px 4px rgba(0,0,0,0.5)' : 'none' }}>
                   {title.replace(/[!！?？]/g, '\n$&')} 
                 </h1>
                 ...
                 <div className="flex flex-wrap justify-center gap-2 mt-4">
                   {tags.slice(0, 3).map((tag, i) => (
                     <span key={i} className={`text-xs px-2 py-1 border rounded-full ${showBackgroundImage ? 'border-white/50 text-primary-text bg-black/20' : 'opacity-80 border-current'}`}>
                       #{tag}
                     </span>
                   ))}
                 </div>
              ...
              <div className="flex flex-col h-full z-10 relative">
                <h2 className={`text-lg font-bold mb-4 border-l-4 pl-3 ${template.accent.replace('bg-', 'border-')} ${showBackgroundImage ? 'text-primary-text drop-shadow-md' : ''}`}>
        ...
        <button
          ...
          className="absolute left-2 top-1/2 -translate-y-1/2 p-2 bg-white/80 rounded-full shadow-sm hover:bg-surface text-text-secondary"
        ...
        <button
          ...
          className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-white/80 rounded-full shadow-sm hover:bg-surface text-text-secondary"
          title="Next Template"
        >
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          disabled={generating}
          className="flex items-center px-4 py-2 bg-text text-primary-text rounded-md text-sm hover:bg-text transition-colors"
        >
          {generating ? '生成中...' : (
            <>
