...
interface NoteEditorProps {
  ...
  cardGeneratorRef: RefObject<CardGeneratorHandle>;
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex justify-between items-center mb-2">
          <span className="text-xs font-semibold text-text-tertiary uppercase tracking-wider">配图生成</span>
          <span className="text-xs text-text-tertiary">点击图片重新生成，悬停可修改提示词</span>
        </div>
        
        {generatedImages.length > 0 ? (
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {/* Card Generator Preview (Always First) */}
            <div className="group relative aspect-square rounded-lg overflow-hidden border border-border shadow-sm hover:shadow-md transition-all bg-surface">
                <div className="absolute top-2 left-2 z-10 bg-black/60 text-primary-text text-[10px] px-1.5 py-0.5 rounded backdrop-blur-sm">
                    封面卡片
                </div>
                <div className="w-full h-full transform scale-90 origin-center">
                 ...
                 <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2 z-20">
                      <button
                        ...
                        className="bg-white/90 text-primary px-3 py-1.5 rounded-full hover:bg-surface transition-colors text-xs font-bold flex items-center"
                      >
            ...
            {generatedImages.map((img, idx) => (
              <div key={idx} className="group relative aspect-square bg-surface-muted rounded-lg overflow-hidden border border-border">
                {img.loading ? (
                  <div className="absolute inset-0 flex flex-col items-center justify-center text-text-tertiary bg-surface-muted">
                  ...
                  <>
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2">
                      <button
                        ...
                        className="bg-white/90 text-text p-1.5 rounded-full hover:bg-surface transition-colors"
                        title="编辑图片 (涂鸦/参考)"
                      >
                        <PenTool size={14} />
                      </button>
                      <button
                        ...
                        className="bg-white/90 text-success p-1.5 rounded-full hover:bg-surface transition-colors"
                        title="设为卡片背景"
                      >
                      ...
                      <button
                        ...
                        className="bg-white/90 text-primary p-1.5 rounded-full hover:bg-surface transition-colors"
                        title="以此图为参考重绘 (变体)"
                      >
                        <Wand2 size={14} />
                      </button>
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center text-text-tertiary hover:bg-surface-hover transition-colors gap-2">
                      <button
                      ...
                      {remixStructure && remixStructure.ref_images && remixStructure.ref_images.length > 0 ? (
                          <div className="flex gap-1 mt-1">
                             {remixStructure.ref_images.slice(0, 3).map((refUrl: string, refI: number) => (
                                 <button
                                    ...
                                    className="w-6 h-6 rounded overflow-hidden border border-primary-subtle hover:border-primary"
                                    title="使用此原图作为参考"
                                 >
                          ...
                          <button
                            ...
                            className="flex flex-col items-center mt-2 text-primary hover:text-primary-hover"
                            title="上传参考图生成"
                          >
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-black/80 text-primary-text text-[10px] p-2 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col gap-1">
                    {editingPromptIdx === idx ? (
                        <div className="flex flex-col gap-1">
                            <textarea 
                                value={tempPrompt}
                                onChange={(e) => setTempPrompt(e.target.value)}
                                className="w-full text text-xs p-1 rounded h-16"
                                onClick={(e) => e.stopPropagation()} 
                            />
                            <div className="flex gap-1 justify-end">
                                <button onClick={(e) => { e.stopPropagation(); setEditingPromptIdx(null); }} className="text-text-tertiary hover:text-primary-text">取消</button>
                                <button onClick={(e) => { e.stopPropagation(); savePrompt(idx); }} className="text-success hover:text-success font-bold">生成</button>
                            </div>
                        </div>
                    ) : (
                        <div 
                            className="cursor-pointer hover:text-text-tertiary truncate" 
                            title="点击修改提示词"
          ...
          <div className="text-center p-4 bg-surface-muted rounded-lg border border-dashed border-border text-text-tertiary text-xs cursor-pointer hover:bg-surface-muted transition-colors" onClick={() => {
              // Add a default placeholder image slot
              handleGenerateImage(0, 'High quality photography, aesthetic, 4k', true);
          }}>
             <ImageIcon size={24} className="mx-auto mb-2 opacity-50" />
             <p>该内容暂无 AI 配图建议</p>
             <p className="mt-1 text-primary">点击此处生成第一张配图</p>
          </div>
        )}
      </div>
    </div>
